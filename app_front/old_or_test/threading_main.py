...
class IoFront(ttk.Frame):
    ...
    def start_recording_button(self):
        ...
        button.bind("<Button-1>", lambda e: [
            threading.Thread(target=self.start_both_recordings).start()
        ])
    ...
    def stop_recording_button(self):
        ...
        button.bind("<Button-1>", lambda e: [
            threading.Thread(target=self.stop_both_recordings).start()
        ])
    ...
    # def start_both_recordings(self):
    #     self.screen_recorder = rec_vid.ScreenRecorder()
    #     self.screen_recorder.start_record()
    #
    #     self.audio_recorder = rec_aud.AudioRecorder()
    ...
    def start_both_recordings(self):
        with ThreadPoolExecutor() as executor:
            executor.submit(self.start_video_recording)
            executor.submit(self.start_audio_recording)
