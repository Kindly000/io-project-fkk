...
class IoFront(ttk.Frame):
    ...
    def process_files_main_app_button(self):
        ...
        button.bind("<Button-1>", lambda x: [self.start_processing_button_main_app()])
    ...
    def on_click_search(self):
        ...
        if (
            com_www_server.get_info_of_notes_from_server_if_note_contain_search_word(
                get_text
            )
            is not None
        ):
    ...
    def stop_recording_button_new_window(self):
        ...
        download_label = ttk.Label(
            new_window, text="Select dir to local download", font=("Arial", 9)
        )
        ...
        button_dir.bind(
            "<Button-1>",
            lambda e: [
                self.open_download_directory_picker(),
        ...
        info_label = ttk.Label(
            new_window,
            text="Download files locally, later still you can make notes from them",
            font=("Arial", 9),
            bootstyle="secondary",
        )
        info_label.pack(pady=10)

        process_recording_label = ttk.Label(
            new_window, text="Process recording", font=("Arial", 9)
        )
        ...
        process_recording_button.bind(
            "<Button-1>", lambda e: [check_file_and_process()]
        )
        check_file_presence()
    ...
    def start_processing_button_new_window(self):
        ...
        entry_title = ttk.Entry(
            new_window, width=30, textvariable=self.existing_record_entered_name
        )
        ...
        label_dir = ttk.Label(
            new_window, text="Choose directory for note", font=("Arial", 9)
        )
        ...
        checkbox = ttk.Checkbutton(
            new_window, variable=self.existing_record_send_to_server_from_new_window_var
        )
            ...
            lambda e: [
                ...
                (
                    self.executor.submit(
                        self.start_data_analyze, self.record_dir, self.date_var
                    )
                    if validate_title()
                    else None
                ),
    ...
    def start_processing_button_main_app(self):
        ...
        def validate_title():
            ...
            if (
                self.existing_wav_file_to_process == ""
                or self.existing_mp4_file_to_process == ""
            ):
                validation_label.config(
                    text="Nie może być pusta",
                    bootstyle="danger",
        ...
        def existing_record_wav_files_directory_picker():
            selected_directory = filedialog.askopenfilename(
                title="Choose file", filetypes=[("WAV files", "*.wav")]
            )
        ...
        def existing_record_mp4_files_directory_picker():
            selected_directory = filedialog.askopenfilename(
                title="Choose file", filetypes=[("MP4 files", "*.mp4")]
            )
            if selected_directory:
    ...
    def save_name_dir_in_variables_existing_record(self):
        ...
        self.existing_record_send_to_server_from_new_window = (
            self.existing_record_send_to_server_from_new_window_var.get()
        )
        ...
        print(
            [
                self.existing_wav_file_to_process,
                self.existing_mp4_file_to_process,
                self.existing_record_file_name,
                self.existing_record_selected_dir_var,
                self.existing_record_app_name,
                self.existing_record_frequency_comparison_sec,
                self.existing_record_send_to_server_from_new_window,
            ]
        )
    ...
    def placeholder(self):
        ...
        self.executor.submit(
            self.start_data_analyze, self.analyze_dir, self.date_var_analyze
        )
